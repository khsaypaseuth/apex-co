---
title: "Tax Registration for New Companies in Laos"
summary: "What tax registration involves for a newly formed Lao company — obtaining a taxpayer identification number, understanding your filing obligations, and getting the accounting foundations right."
lastUpdated: "2026-07-11"
readingTime: 5
category: "tax-accounting"
verificationStatus: "needs-verification"
relatedServices: ["accounting-tax", "business-setup"]
relatedArticles:
  [
    "company-registration-in-laos-basic-steps",
    "vat-in-laos-simple-overview",
    "corporate-income-tax-in-laos-simple-overview",
    "annual-compliance-checklist-for-companies-in-laos",
  ]
sources: []
---

Once a company is registered in Laos, it must enter the tax system. Tax registration gives the company a taxpayer identification number (TIN) and places it under the administration of the tax authority, which determines what the company must file and when. Getting this step right — and setting up the bookkeeping to support it — is the foundation of staying compliant.

> **Needs legal verification before publication.**

## Who needs to register

In general, every enterprise operating in Laos needs to be registered for tax, regardless of size or ownership. This includes:

- Newly incorporated limited companies (local or foreign-owned)
- Branches and representative offices of foreign companies
- Sole traders and small enterprises, which may fall under simplified regimes depending on their scale

Which regime applies to a given business — and what that means for filings — depends on factors such as turnover and activity, and should be confirmed with the tax authority for your specific case.

## What registration generally involves

1. **Apply for a taxpayer identification number** after (or alongside) enterprise registration.
2. **Provide company information** — the enterprise registration certificate, registered address, activity description, and responsible persons are commonly requested.
3. **Confirm which taxes apply to you.** Depending on the business, this can include profit (corporate income) tax, value added tax, withholding obligations, and payroll-related obligations for employees.
4. **Understand your filing calendar.** Different taxes have different filing and payment cycles; the current schedule should be confirmed with the tax authority.
5. **Set up compliant bookkeeping** so filings can actually be prepared from reliable records.

> **Needs legal verification before publication.**

## Obligations that commonly follow registration

| Obligation | What it generally means |
| --- | --- |
| Periodic tax filings | Returns filed on the cycle set for each tax type |
| Payroll obligations | Withholding and social security contributions for employees |
| Accounting records | Books maintained in line with Lao accounting requirements |
| Annual obligations | Year-end financial statements and annual tax finalisation |

The specific set of obligations depends on your regime, sector, and whether you have employees. Treat this table as orientation, not a compliance list.

## Common mistakes

- **Registering and then going quiet.** Filing obligations usually start immediately, even for a company with little or no activity yet — nil returns may still be required.
- **No bookkeeping until the first deadline.** Reconstructing months of records under deadline pressure is expensive and error-prone.
- **Ignoring payroll obligations.** Hiring the first employee triggers withholding and social security registration questions that are easy to miss.
- **Assuming the regime never changes.** Growth can move a business between regimes; this should be reviewed as the business scales.

## Rates and thresholds

Tax rates, registration thresholds, and penalty amounts are set by law and change over time. We deliberately do not publish figures here — current rates and thresholds should be confirmed with the tax authority or a qualified adviser before making decisions.

## How SV Consulting can help

SV Consulting handles tax registration as part of company setup, confirms which obligations apply to your specific business, sets up compliant bookkeeping from day one, and manages the filing calendar so deadlines are met without drama.

*The site-wide disclaimer applies to this article: it is general information, not legal, tax, or immigration advice.*
