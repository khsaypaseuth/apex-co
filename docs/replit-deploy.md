# Deploy Super Consulting on Replit

## Prerequisites

- GitHub repo: `khsaypaseuth/super-consulting`
- Node.js 22+
- Custom domain: `superconsulting.la` (and optionally `www.superconsulting.la`)
...
4. Attach the custom domain in Replit **Hosting / Domains**:
   - Apex: `superconsulting.la`
   - Optional: `www.superconsulting.la` → redirect or alias to apex
...
6. Confirm HTTPS is issued, then open:
   - https://superconsulting.la/en
   - https://superconsulting.la/sitemap.xml
   - https://superconsulting.la/llms.txt
   - https://superconsulting.la/robots.txt

## Site URL

[`lib/site-config.ts`](../lib/site-config.ts) sets `SITE_URL = 'https://superconsulting.la'`.
